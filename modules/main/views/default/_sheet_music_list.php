...
<tr>
    ...
    <td>
        <div class="heading-5">
            <?php
                if ($model->type == SheetMusic::TYPE_BIG_BAND)
                    echo Html::a($model->name, ['/big-band-view/' . $model->id]);
                if ($model->type == SheetMusic::TYPE_JAZZ_COMBO)
                    echo Html::a($model->name, ['/jazz-combo-view/' . $model->id]);
                if ($model->type == SheetMusic::TYPE_POP_MUSIC)
                    echo Html::a($model->name, ['/pop-music-view/' . $model->id]);
            ?>
        </div>
    </td>
    <td>
